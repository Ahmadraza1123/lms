...
class BookViewSet(viewsets.ModelViewSet):
    ...
    def borrow(self, request, pk=None):
        ...
        if getattr(user, 'role', None) in ['librarian', 'admin'] or user.is_staff or user.is_superuser:
            return Response(
                {"detail": "Only members can borrow books. Librarians/Admins are not allowed."},
    ...
    def borrowedlist(self, request):

        borrows = BorrowRecord.objects.filter(user=request.user, returned=False)
